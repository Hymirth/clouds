...
#var perlin_texture_2d : Texture2DRD
...
func _init() -> void:
	...
	file_2d = load("res://shaders/perlin_noise.glsl")
	file_3d = load("res://shaders/perlin_noise_3d.glsl")
...
func generate_2d(size: int, frequency : float, octaves : float, offset : float = 0.0) -> Texture2DRD:
	...
	var perlin_texture_2d = Texture2DRD.new()
	perlin_texture_2d.texture_rd_rid = _create_rd_texture(size)
	
	var texture_uniform = RDUniform.new()
	texture_uniform.uniform_type = RenderingDevice.UNIFORM_TYPE_IMAGE
	texture_uniform.binding = 0
	texture_uniform.add_id(perlin_texture_2d.texture_rd_rid)
...
func generate_3d(size: int, frequency : float, octaves : float, offset : float = 0.0) -> Texture3DRD:
	...
	var perlin_texture_3d = Texture3DRD.new()
	perlin_texture_3d.texture_rd_rid = _create_rd_texture(size, true)
	
	var texture_uniform = RDUniform.new()
	texture_uniform.uniform_type = RenderingDevice.UNIFORM_TYPE_IMAGE
	texture_uniform.binding = 0
	texture_uniform.add_id(perlin_texture_3d.texture_rd_rid)
