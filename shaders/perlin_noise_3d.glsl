...
void main() {
    ...
    ivec3 size = imageSize(color_image);
    if (uv.x >= size.x || uv.y >= size.y || uv.z >= size.z) return;

    vec3 norm_uv = vec3(uv) / vec3(size);
    ...
    {
      total += amp * pnoise(frequency * offset_pos, vec3(frequency));
      frequency *= 2.0;
      amp *= 0.5;
    }
